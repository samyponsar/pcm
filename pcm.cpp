...
class Osc{
		...
		int mode = 0;
		double duration = 1, frequency = 1, amplitude = 1;
...
void Osc::init(){
	cout << "Osc mode? (1=sine | 2=triangle | 3=square | 4=saw | 5=reverse saw | 0=silence) ";
	while ( !(cin >> mode) || !(mode == 0 || mode == 1 || mode == 2 || mode == 3 || mode == 4 || mode == 5) ) {
		cin.clear();
		cin.ignore(1000, '\n');
		cout << "Invalid input.\nOsc mode? (1=sine | 2=triangle | 3=square | 4=saw | 5=reverse saw | 0=silence) ";
	}
	cout << "Duration? (max. 3600 seconds) ";
	while(!(cin >> duration) || duration < 0 || duration > 3600) {
		cin.clear();
		cin.ignore(1000, '\n');
		cout << "Invalid input.\nDuration? (max. 3600 seconds) ";
	}
	if (mode != 0) {
		cout << "Frequency? (Hz) ";
